...
template<typename T>
int max_node(Node<T>* ptr) {
    if (ptr == nullptr) { return 0; }
    if (ptr->left == nullptr && ptr->right == nullptr) { return ptr->data; }
    return max(max_node(ptr->left), max_node(ptr->right));
}

template<typename T>
int min_node(Node<T>* ptr) {
    if (ptr == nullptr) { return 99999; }
    if (ptr->left == nullptr && ptr->right == nullptr) { return ptr->data; }
    return min(min_node(ptr->left), min_node(ptr->right));
}