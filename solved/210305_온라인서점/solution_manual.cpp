...
struct PriorityQueueAuthor{
	priority_queue<Author> heap;

	void init() { while (!heap.empty()) { heap.pop(); } }
	void push(const Author& author) { heap.push(author); }
	void pop() { heap.pop(); }
	Author top() { return heap.top(); }
	bool empty() { return heap.empty(); }
};
...
struct PriorityQueueAuthor {
	Author heap[NUM_AUTHORS];
	int heapSize = 0;

	void init() { heapSize = 0; }
	void push(const Author& author) {
		heap[heapSize] = author;
		int current = heapSize;

		while (current > 0 && heap[(current - 1) / 2] < heap[current])
		{
			Author temp = heap[(current - 1) / 2];
			heap[(current - 1) / 2] = heap[current];
			heap[current] = temp;
			current = (current - 1) / 2;
		}
	...
	void pop() {
		...
		while (current * 2 + 1 < heapSize)
		{
			...
			Author temp = heap[current];
			heap[current] = heap[child];
			heap[child] = temp;
			current = child;
		}
	}
	Author top() { return heap[0]; }
	bool empty() { return heapSize == 0; }
};
#endif
...
int getBestSeller(int cID, int bIDs[]) {
	PriorityQueueBook& Q = categories[cID].bookPQ;
	LinkedList popped;
	int cnt = 0;
