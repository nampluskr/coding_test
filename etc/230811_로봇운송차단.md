...
void setCandidate(int y, int x) {
	for (int d = 0; d < 2; d++) {
		for (int len = 2; len <= 5; len++) {
			...
			if (ey >= mapSize || ex >= mapSize)
				continue;

			int key = getCandidateKey(y, x, len, d);
			
			if (candidateMap.find(key) == candidateMap.end())// 최초 push라면 vector 추가해준다.
				candidateMap.emplace(key, vector<Candidate>());
...
int getCandidate(int len, char structure[5]) {
	...
	for (int dir = 0; dir < 2; dir++) {
		...
		if (candidateMap.find(key) == candidateMap.end())
			continue;
		
		for (auto c : candidateMap[key]) {
			candidateList[wp++] = c;
		}
	}
	return wp;
	...
	while (cQueue.size() > 0) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (ny < 0 || nx < 0 || ny >= mapSize || nx >= mapSize)
				continue;
			if (!isblocked[ny][nx] || map[p.first][p.second] > map[ny][nx])
				continue;
...
int maxBlockedRobots(int M, int mStructure[], int mDir)
{
	...
	for (int i = 0; i < loop; i++) {
		...
		for (int j = 0; j < candidateCount; j++) {
			setStructure(M, structure[i], candidateList[j], INSTALL);
			int numOfBlockedRobots = getBlockedRobots(mDir);
			
			if (numOfBlockedRobots > maxNumOfBlockedRobots)
				maxNumOfBlockedRobots = numOfBlockedRobots;

			setStructure(M, structure[i], candidateList[j], UNINSTALL);
		}
	}
